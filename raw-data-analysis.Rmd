...
annotatedData <- new("LumiBatch", exprs=exprs(rawData), phenoData = adf,
                  ...
                  detection = detection(rawData), featureData = featureData(rawData))
...
get_df <- function(genes) {
   f <- fData(expressedProbes.lumi)
   f[f$TargetID %in% genes, c("ProbeID", "TargetID")]
}

probes <- get_df(c("IL10", "PNPT1", "PAK2", "TRIM5", "DSP",
   "NFAT5", "TGFBRAP", "CCL19", "CCL21", "SPINK4", "CD7", 
   "CCR7", "CCL23", "CXCL9", "MMP12"))

d <- exprs(expressedProbes.lumi)
...
library(ggplot2)
ggplot(selected, aes(x = Concentration, y = Log2Expression, color = Donor)) + 
   geom_point() + 
